...
class GraphVerifier:
    def __init__(self, kg_path: str):
        ...
        self.backlinks: Dict[str, Set[str]] = defaultdict(set)  # node -> nodes that link to it
    ...
    def scan_file(self, file_path: Path, is_node: bool = False) -> None:
        ...
        if not frontmatter:
            if is_node:
                self.errors.append(f"Node {file_path.name}: Missing or invalid frontmatter")
            return
    ...
    def check_broken_links(self) -> None:
        ...
        for broken_link in sorted(broken_links):
            self.errors.append(f"Broken link: node:{broken_link} (target does not exist)")
    ...
    def check_duplicate_ids(self) -> None:
        ...
        for node_id, files in self.duplicate_ids.items():
            if len(files) > 1:
                ...
                self.errors.append(f"Duplicate node ID '{node_id}' in files: {file_list}")
