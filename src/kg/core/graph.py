...
class GraphTraverser:
    ...
    def bfs_traverse(self, start_id: str, max_depth: int = 3, direction: str = "both") -> Dict[int, List[str]]:
        """Breadth-first traversal from a starting node."""
        # Check if it's a virtual node (like sprint:2025-08-04)
        if start_id not in self.nodes and start_id not in self.reverse_edges and start_id not in self.edges:
    ...
    def find_path(self, start_id: str, end_id: str, max_length: int = 6) -> Optional[List[str]]:
    ...
    def export_graphviz(self, output_file: str = "graph.dot", highlight_nodes: Set[str] = None):
...
def main():
    ...
    parser.add_argument("--centrality", action="store_true", help="Show node centrality")
    ...
    if args.traverse:
        ...
        for node_id, total, in_deg, out_deg in centrality[:20]:
            node = traverser.nodes.get(node_id, {})
            print(f"{node_id:30} {total:3} ({in_deg:2} in, {out_deg:2} out) - {node.get('title', '')[:40]}")
        ...
        if args.traverse:
            traversal = traverser.bfs_traverse(args.traverse, args.depth, args.direction)
            highlight = set(node_id for nodes in traversal.values() for node_id in nodes)
